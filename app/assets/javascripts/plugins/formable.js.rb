module Plugins
	module Formable  
      ...
      #INPUT element should implement the needed inteface
			...
			options[:model] = model
			options[:attr] = attr
      #this is neede for collecting inputs by namespaces
      ...
      if n_state(f_m)
  	    n_state(f_m).validate(only: options[:validate_only]) unless (options[:validate] == false)
  	    n_state(f_m)
      end
      model = options[:model]
      if model
        model.validate(only: options[:validate_only]) unless (options[:validate] == false)
      end
    ...
    #view helper for diplaying all errors if they are not empty and exist on model
    # model : Model
    def display_all_errors_for(model)
      if model.errors
        t(:div, {className: 'general_errors'},
          model.errors.map do |k, v|
            t(:p, {}, "#{v}")
          end
        )
      end
    end
    #refer to #display_all_errors_for, the difference is that it only render 'general' errors
    def display_general_errors_for(model)
      if model.has_errors? && model.errors[:general]
        t(:div, {className: 'general_errors'},
          model.errors[:general].map do |v|
            t(:p, {}, "#{v}")
          end
        )
      end
		...
		def clear_inputs(name_space = '')
      #clears inputs if they respond to :clear_inputs
			refs.each do |k,v|
				if k.include? "_input_#{name_space}"
